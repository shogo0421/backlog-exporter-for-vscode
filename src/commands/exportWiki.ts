...
export async function exportWiki(configManager: ConfigManager, outputChannel: vscode.OutputChannel): Promise<void> {
  ...
  const outputDir = resolveOutputDir(workspaceFolder, exportSettings.outputDirectory, 'wiki')

  await vscode.window.withProgress(
    {
      cancellable: false,
      location: vscode.ProgressLocation.Notification,
      title: 'Backlog: Wiki をエクスポート中...',
    },
    async (progress) => {
      try {
        ...
        vscode.window.showInformationMessage('Backlog: Wiki のエクスポートが完了しました。')
      } catch (error) {
