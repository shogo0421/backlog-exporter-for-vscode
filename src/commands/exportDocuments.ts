...
export async function exportDocuments(
  ...
  const outputDir = resolveOutputDir(workspaceFolder, exportSettings.outputDirectory, 'documents')

  await vscode.window.withProgress(
    {
      cancellable: false,
      location: vscode.ProgressLocation.Notification,
      title: 'Backlog: Documents をエクスポート中...',
    },
    async (progress) => {
      try {
        ...
        vscode.window.showInformationMessage('Backlog: Documents のエクスポートが完了しました。')
      } catch (error) {
